...
Table: tenant_encryption_keys (NEW)
  tenant_id         UUID PK                    -- sentinel row for the NULL-tenant default bucket, see Implementation Notes
...
| `TenantKeyService` (new) | `src/services/__tests__/tenantKey.service.test.ts` | Generate + wrap DEK per tenant; unwrapped-DEK cache hit/miss (bounded LRU); default-bucket fallback for `tenant_id IS NULL`; rotate a tenant's DEK without re-encrypting existing rows; two tenants' ciphertexts are not cross-decryptable |
| `ValidationSchemas` (partial) | `src/schemas/__tests__/validation.test.ts` | Login schema rejects missing fields; refresh schema accepts optional `idempotency_key` as UUID |

### Integration Tests

- `POST /v1/auth/login` — covers AC-1, AC-2. Test setup: seeded user fixture with known bcrypt hash.
- `POST /v1/auth/refresh` — covers AC-7, AC-8, AC-10. Verify `refresh_tokens.revoked_at` and `session_family` behavior; verify duplicate `idempotency_key` submission within the window returns the identical pair; verify the cache expiring after the window falls back to normal reuse detection.
...
- File path: `src/__tests__/integration/auth.test.ts`
...
- **Idempotency cache (Open Question 2) — flagged for security review:** the cache backing `idempotency_key` retries necessarily holds a plaintext issued token pair (access + refresh token) for its ~30s TTL, in addition to the copy already returned to the client. This is a new location where live tokens exist, even briefly, and should be scoped explicitly by the `security-reviewer` subagent at `/plan`/`/implement` time per this file's Security Rules (e.g., storage location, TTL tightness, and whether the cached value should itself be encrypted at rest in Redis) rather than assumed safe by default.
