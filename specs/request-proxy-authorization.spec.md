...
- `RouteConfigSchema` — validates each entry of the `routes` JSON file at startup: `{ path, upstream: string (url), auth_required: boolean, required_scope: string | null, allowed_roles: string[] | null, rate_limit_override: number | null, strip_prefix: boolean, timeout_ms: number }`, new file `src/schemas/route.schemas.ts`
...
| `ValidationSchemas` (partial) | `src/schemas/__tests__/validation.test.ts` | Route config schema rejects malformed route entries |

### Integration Tests

- `ALL /api/:service/*` — covers AC-25, AC-26, AC-27, AC-32. Scenarios: authenticated pass-through with header injection, unauthenticated on public route, unauthenticated on private route → 401, insufficient role → 403, insufficient scope → 403, rate limit → 429, upstream timeout → 504, header stripping.
- File path: `src/__tests__/integration/proxy.test.ts`
...
|---|----------|-------|-----|------------|
| 1 | What upstream services should be included in the initial route configuration for the first deployment? The proxy spec is generic, but real route entries depend on the platform service registry. | Backend Platform | Sprint 2 | *Pending* |
| 2 | Should `audit_log` entries for `proxy.request_allowed` be written for every single proxied request, or only for sensitive routes? High-traffic proxies may generate audit log volumes that overwhelm the PostgreSQL write capacity. Consider sampling or async write via Redis queue. | DevOps / Platform | Sprint 2 | *Pending* |
| 3 | Is there a requirement for the gateway to support mutual TLS (mTLS) for upstream service communication, or is internal network trust sufficient for the initial deployment? | Security Engineering | Sprint 1 | *Pending* |
